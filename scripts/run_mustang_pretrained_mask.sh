...
# Example: meta-train MUSTANG, then fine-tune with pretrained masking.
# Usage:
#   bash scripts/examples/run_mustang_then_finetune_pretrained_mask.sh [target_dataset] [pretrain_missing_rate]
# Example:
#   bash scripts/examples/run_mustang_then_finetune_pretrained_mask.sh ssc 0.9

TARGET_DATASET="${1:-ssc}"
PRETRAIN_MISSING_RATE="${2:-0.9}"
...
TRAIN_MISSING="point"
TEST_MISSING="point"
# Kept for compatibility, but ignored when --pretrain_missingrate > 0.
...
echo "[2/2] Fine-tuning with pretrained masking"
echo "      target=${TARGET_DATASET}, pretrain_missingrate=${PRETRAIN_MISSING_RATE}"
python main_conditional_diffusion.py \
  ...
  --pretrain_missingrate "${PRETRAIN_MISSING_RATE}" \
...
echo "Pretrained masking rate used in finetune/eval: ${PRETRAIN_MISSING_RATE}"
